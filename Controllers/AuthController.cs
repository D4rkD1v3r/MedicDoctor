...
namespace rmiMedicineDoctor.Controllers
{
  ...
  public class AuthController : Controller
  {
    ...
    public async Task<IActionResult> Post([FromBody]CredentialsViewModel credentials)
    {
      ...
      if (identity == null)
      {
        return BadRequest(Errors.AddErrorToModelState("login_failure", "Неверное имя пользователя или пароль.", ModelState));
      }

      // Serialize and return the response
      var response = new
      {
        id = identity.Claims.Single(c => c.Type == ClaimConsts.IdClaimName).Value,
